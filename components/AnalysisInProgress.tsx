...
interface AnalysisInProgressProps {
    imageUrl: string;
    phase: AnalysisPhase;
    t: (key: 'phaseInitialization' | 'phaseFeatureExtraction' | 'phaseHypothesisGeneration' | 'phaseSynthesis') => string;
}

const AnalysisInProgress: React.FC<AnalysisInProgressProps> = ({ imageUrl, phase, t }) => {
    const getProgressClass = () => {
        switch (phase) {
            case 'initialization': return 'w-[5%]';
            case 'featureExtraction': return 'w-[40%]';
            case 'hypothesisGeneration': return 'w-[75%]';
    ...
    const getPhaseText = () => {
        switch (phase) {
            case 'initialization': return t('phaseInitialization');
            case 'featureExtraction': return t('phaseFeatureExtraction');
            case 'hypothesisGeneration': return t('phaseHypothesisGeneration');
    ...
    return (
        <div className="w-full mt-8 flex flex-col items-center animate-fade-in">
            <div className="relative w-full max-w-lg mx-auto overflow-hidden rounded-lg shadow-lg border-2 border-cyan-500/50 group animate-pulse-border">
                <img src={imageUrl} alt="Analyzing image" className="w-full h-auto object-contain max-h-[50vh] transition-transform duration-500 group-hover:scale-105" />
